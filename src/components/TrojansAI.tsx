...
//  Constants
// ============================================================
const LOCAL_CONVOS_KEY = "trojans-ai-convos";
const WELCOME = "Hey! I'm Trojans AI. What can I help you with?";
...
const SUGGESTIONS = [
  ...
  { icon: Code2, text: "Write a Python JSON parser from an API endpoint" },
  { icon: Lightbulb, text: "Creative startup ideas in the AI space" },
  { icon: Sparkles, text: "Dark sci-fi story about a rogue AI" },
];

const SYSTEM_PROMPT = `You are Trojans AI, a helpful and friendly AI assistant. Keep responses concise and natural. When answering educational or factual questions, format your response as:
...
// ============================================================
//  ScreenWidget — Picture-in-Picture screen sharing with AI
// ============================================================
function ScreenWidget({ onClose }: { onClose: () => void }) {
  ...
  const openPiP = async () => {
    ...
    try {
      ...
      doc.documentElement.style.cssText = "height:100%;margin:0;padding:0;box-sizing:border-box;";
      ...
      style.textContent = `
        * { box-sizing:border-box; }
        ...
        .header { display:flex;align-items:center;justify-content:space-between;padding:10px 12px;flex-shrink:0;background:rgba(255,255,255,0.02);border-bottom:1px solid rgba(255,255,255,0.05); }
        .header-left { display:flex;align-items:center;gap:8px; }
        .status-dot { width:7px;height:7px;border-radius:50%;background:rgba(255,255,255,0.2);flex-shrink:0;transition:background 0.3s,box-shadow 0.3s; }
        ...
        .icon-btn { width:26px;height:26px;border-radius:7px;border:none;background:transparent;color:rgba(255,255,255,0.35);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background 0.15s,color 0.15s;padding:0; }
        ...
        .preview-wrap { border-radius:10px;overflow:hidden;border:1px solid rgba(255,255,255,0.07);background:#000;flex-shrink:0;position:relative; }
        .preview-wrap video { width:100%;max-height:150px;object-fit:cover;display:block; }
        .preview-label { position:absolute;bottom:6px;left:8px;font-size:9px;font-weight:600;letter-spacing:0.05em;text-transform:uppercase;color:rgba(255,255,255,0.4);background:rgba(0,0,0,0.5);padding:2px 6px;border-radius:4px; }
        .placeholder { display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px;padding:20px 12px;text-align:center;flex-shrink:0;border-radius:10px;border:1px dashed rgba(255,255,255,0.08);background:rgba(255,255,255,0.02); }
        ...
        textarea { width:100%;padding:9px 36px 9px 11px;border-radius:10px;border:1px solid rgba(255,255,255,0.08);background:rgba(255,255,255,0.04);color:rgba(255,255,255,0.85);font-size:12px;outline:none;resize:none;font-family:inherit;line-height:1.4;min-height:56px;max-height:100px;transition:border-color 0.15s; }
        textarea:focus { border-color:rgba(100,160,255,0.35); }
        textarea::placeholder { color:rgba(255,255,255,0.2); }
        .send-icon-btn { position:absolute;bottom:8px;right:8px;width:22px;height:22px;border-radius:6px;border:none;background:rgba(80,140,255,0.25);color:rgba(160,200,255,0.9);cursor:pointer;display:flex;align-items:center;justify-content:center;transition:background 0.15s;padding:0; }
        ...
        .btn { flex:1;padding:8px 10px;border-radius:9px;font-size:11px;font-weight:600;cursor:pointer;font-family:inherit;transition:all 0.15s;display:flex;align-items:center;justify-content:center;gap:5px;border:none; }
        .btn:disabled { opacity:0.35;cursor:not-allowed; }
        .btn-share { background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.09)!important;color:rgba(255,255,255,0.7); }
        .btn-share:hover:not(:disabled) { background:rgba(255,255,255,0.09); }
        .btn-share.active { background:rgba(239,68,68,0.12);border-color:rgba(239,68,68,0.25)!important;color:rgba(255,110,110,0.9); }
        .response-wrap { flex:1;overflow-y:auto;border-radius:10px;border:1px solid rgba(255,255,255,0.06);background:rgba(255,255,255,0.025);padding:10px 11px;min-height:0; }
        .response-text { font-size:12px;color:rgba(255,255,255,0.75);line-height:1.65;white-space:pre-wrap; }
        .response-error { color:rgba(255,110,110,0.8); }
        .thinking-row { display:flex;align-items:center;gap:6px;font-size:11px;color:rgba(255,255,255,0.3);flex-shrink:0;padding:2px 0; }
        .dot-pulse span { display:inline-block;width:3px;height:3px;border-radius:50%;background:rgba(255,255,255,0.3);animation:dp 1s infinite; }
      ...
      doc.body.innerHTML = `
        <div class="header">
          <div class="header-left">
            ...
            <span class="header-title">Trojans AI Screen Assistant</span>
          </div>
      ...
      const setSharing = (active: boolean) => {
        ...
        shareBtn.innerHTML = active ? `${stopSvg} Stop Sharing` : `${screenSvg} Share Screen`;
      ...
      shareBtn.addEventListener("click", async () => {
        ...
        try {
          pipStream = await pipWin.navigator.mediaDevices.getDisplayMedia({ video: { frameRate: 5 } });
      ...
      const doAsk = async () => {
        ...
        const q = input.value.trim() || "What do you see on my screen? Be concise.";
        ...
        >
          {msg.imageBase64 && msg.imageMime && (
            <img
              ...
              style={{ maxHeight: 120, maxWidth: 200, borderRadius: 8, display: "block", marginBottom: 6 }}
              alt="attachment"
            />
          )}
          {msg.content}
        ...
        {[
          ...
          <button
            ...
            className={`p-1.5 rounded-lg transition-colors ${
              (btn as any).active ? "text-foreground" : "text-foreground/25 hover:text-foreground/60"
            }`}
...
//  Main component
// ============================================================
export default function TrojansAI() {
  ...
  const refreshConvoList = useCallback(() => {
  ...
  const persistConversation = useCallback((hist: HistoryEntry[], convoId: string | null) => {
    const textMsgs = hist
    ...
    const preview = textMsgs.find((m) => m.role === "user")?.content.slice(0, 80) || "New chat";
  ...
  const openConversation = useCallback((id: string) => {
    ...
    setMessages([
      ...
      ...msgs.map((m, i) => ({
        ...
        content: m.role === "assistant" ? formatAiResponse(m.content) : m.content,
      })),
    ]);
    setHistory([
  ...
  useEffect(() => {
    ...
    syncModel();
    window.addEventListener("trojans-settings-updated", syncModel);
    return () => window.removeEventListener("trojans-settings-updated", syncModel);
  }, []);
  ...
  const sendMessage = useCallback(
    async (
      ...
      for (const entry of newHistory) {
        if (entry.role === "user") {
          const isLastUser = entry === [...newHistory].reverse().find((e) => e.role === "user");
          if (entry.image && isLastUser) {
            groqMessages.push({
              role: "user",
              content: [
                { type: "text", text: entry.content || "What's in this image?" },
                {
                  type: "image_url",
                  image_url: { url: `data:${entry.image.mime};base64,${entry.image.base64}` },
                },
              ],
            });
      ...
      try {
          ...
          const errMsg =
            typeof data?.detail === "string"
              ...
              : typeof data?.error === "string"
                ? data.error
                : `Couldn't reach Trojans AI (${res.status})`;
        ...
        setHistory((prev) => {
          const next = [...prev, { role: "assistant" as const, content: aiResponse }].slice(-40);
            ...
            ...prev,
            {
              ...
              content:
                err?.name === "ApiError" && err?.message
                  ? err.message
                  : "Couldn't reach Trojans AI. Try again.",
            },
          ]);
        }
      } finally {
  ...
  const handleRegen = useCallback(() => {
    ...
    if (!lastUser) return;
    const img = lastUser.image ? { base64: lastUser.image.base64, mime: lastUser.image.mime } : null;

    let trimmed = history;
    if (trimmed.length && trimmed[trimmed.length - 1]?.role === "assistant") trimmed = trimmed.slice(0, -1);
  ...
  const handleFile = (file: File) => {
    ...
    reader.onload = (e) => {
      ...
      img.onerror = () => setPendingImage({ base64: result.split(",")[1], mime: file.type });
      img.src = result;
    };
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 pt-2">
        <div className="flex items-center gap-1.5 relative">
          ...
          <button
            type="button"
            onClick={() => setShowScreenWidget(true)}
            title="Share screen"
            className="w-8 h-8 rounded-lg flex items-center justify-center"
            style={{
              background: "hsla(0,0%,100%,0.05)",
  ...
  return (
    <div className="absolute inset-0 flex overflow-hidden">
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
          ...
          <>
            <div className="absolute top-3 left-3 z-30 flex items-center gap-1.5">
              <button
                ...
                style={{ background: "hsla(220, 28%, 8%, 0.55)", border: "1px solid hsla(0,0%,100%,0.1)", cursor: "pointer", backdropFilter: "blur(12px)" }}
              >
                <History size={14} />
              </button>
            </div>
              ...
              >
                <div className="text-center space-y-2">
                  <h2
                    ...
                    style={{ color: "hsla(0,0%,98%,0.96)", letterSpacing: "-0.03em", textShadow: "0 2px 24px rgba(0,0,0,0.45)" }}
                  >
                    Trojans AI
                  </h2>
                  <p className="text-[13px] min-h-[1.25rem]" style={{ color: "hsla(0,0%,100%,0.62)", textShadow: "0 1px 12px rgba(0,0,0,0.4)" }}>
                    ...
                    <span
                      ...
                      style={{ background: "hsla(0,0%,100%,0.45)", animation: "pz-caret 1s step-end infinite" }}
                    />
                  </p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 w-full mt-2">
                  {SUGGESTIONS.map((s, i) => (
                    <button
                      ...
                      className="flex items-center gap-2.5 text-left px-3 py-2.5 rounded-xl text-[12px] transition-colors"
                      style={{
                    ...
                    >
                      <s.icon size={13} style={{ flexShrink: 0, opacity: 0.7 }} />
            ...
            >
              <div className="flex items-center gap-3 min-w-0">
                <h1 className="text-sm font-semibold text-foreground truncate">
                  {convos.find((c) => c.id === activeConvoId)?.title || "Trojans AI"}
                </h1>
              </div>
        ...
        <AnimatePresence>{showScreenWidget && <ScreenWidget onClose={() => setShowScreenWidget(false)} />}</AnimatePresence>
      </div>
            ...
            >
              <div className="flex items-center justify-between px-4 pt-4 pb-2">
                <div>
                  ...
                  <p className="text-[11px] text-white/40">Stored locally on this device</p>
                </div>
                <button
                  type="button"
                  onClick={() => setHistoryOpen(false)}
